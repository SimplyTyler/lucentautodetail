...
const demoCustomers = [
  { id: "admin-user-1", name: "Alex Morgan", email: "alex@example.com", phone: "(602) 555-0148", admin_notes: "Prefers text confirmation. Side gate code is stored on each visit.", created_at: timestampFromNow(-70), membership_id: "admin-membership-1", plan_code: "reserve", vehicle_count: 2, membership_status: "active", cancel_at_period_end: false, current_period_end: timestampFromNow(21), vehicle_total: 2, last_visit_at: dateFromNow(-25) },
  { id: "admin-user-2", name: "Northline Realty", email: "ops@northline.example", phone: "(480) 555-0194", admin_notes: "Coordinate grouped service with facilities before arrival.", created_at: timestampFromNow(-46), membership_id: "admin-membership-2", plan_code: "fleet", vehicle_count: 12, membership_status: "active", cancel_at_period_end: false, current_period_end: timestampFromNow(18), vehicle_total: 3, last_visit_at: dateFromNow(-8) },
  { id: "admin-user-3", name: "Jordan Ellis", email: "jordan@example.com", phone: "(623) 555-0186", admin_notes: null, created_at: timestampFromNow(-8), membership_id: "admin-membership-3", plan_code: "drive", vehicle_count: 1, membership_status: "trialing", cancel_at_period_end: false, current_period_end: timestampFromNow(6), vehicle_total: 1, last_visit_at: null },
  { id: "admin-user-4", name: "Summit Executive", email: "fleet@summit.example", phone: "(602) 555-0121", admin_notes: "Billing contact is reviewing an expired card.", created_at: timestampFromNow(-120), membership_id: "admin-membership-4", plan_code: "fleet", vehicle_count: 7, membership_status: "past_due", cancel_at_period_end: false, current_period_end: timestampFromNow(-2), vehicle_total: 1, last_visit_at: dateFromNow(-12) },
  { id: "admin-user-5", name: "Maya Chen", email: "maya@example.com", phone: "(480) 555-0162", admin_notes: "Requested a collector-plan consultation.", created_at: timestampFromNow(-3), membership_id: null, plan_code: null, vehicle_count: 0, membership_status: null, cancel_at_period_end: false, current_period_end: null, vehicle_total: 1, last_visit_at: null }
];
...
function metricsFor(customers, vehicles, memberships, appointments) {
...
export async function getAdminWorkspaceData() {
  if (!hasDatabase()) {
    return {
      ...
      metrics: metricsFor(demoCustomers, demoVehicles, demoMemberships, demoAppointments)
    };
  }

  const [customersResult, vehiclesResult, membershipsResult, appointmentsResult] = await Promise.all([
    query(
      `SELECT u.id, u.name, u.email, u.phone, u.admin_notes, u.created_at,
  ...
  return {
    ...
    memberships,
    appointments,
    metrics: metricsFor(customers, vehicles, memberships, appointments)
  };
}
